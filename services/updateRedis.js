...
async function main() {
  try {
    const { from, to, today, tomorrow } = getTodayRangeUTC();

    await redisClient.connect();

    const response = await axios.get(
      "https://api.betmines.com/betmines/v1/fixtures/web",
      {
        headers: {
    ...
    const partidos = partidosFiltrados.map((f) => {
      const found = existingData.find((p) => p.apiId === f.id);
    ...
    await redisClient.setEx(redisKey, 60 * 60 * 2, JSON.stringify(partidos));
